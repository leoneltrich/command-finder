...
impl<S: StoragePort> CatalogIngestionPort for CatalogLifecycleManager<S> {
    fn ingest_catalog(&self, catalog: &OptimizedToolCatalog, _auth_key: &str) -> Result<bool, AppError> {
        self.storage_port.save_catalog(catalog)
    }

    fn update_catalog(&self, catalog: &OptimizedToolCatalog, _auth_key: &str) -> Result<bool, AppError> {
